 ...
 */
    ...
    .factory('GAPI', function ($q, $http, $log, GoogleApp) {
      ...
      GAPI.init = function (cordova) {

        var app = GAPI.app,
            deferred = $q.defer(),
            attemptCounter = 0,
            endPoint = cordova || gapi.auth.authorize,
            ...
            onAuth = function (response) {
              ...
              if(response && !response.error) {
                ...
                }else if(cordova && response && response.access_token){
                  app.oauthToken = response;
                  deferred.resolve(app);
                }
                else {
        ...
        if(app.oauthToken && app.oauthToken.access_token && app.oauthToken.expires_in ){
          deferred.resolve(app);
          return;
        }
        deferred.notify('Trying to log-in to Google API services.');
        var auth = function(_immediate){
          gapi.auth.authorize({
          ...
          var cordovarAuth =  function(){
            cordova(app.clientId, app.scopes, {clearsessioncache: 'yes', clearcache: 'yes', approval_prompt: 'auto'}).then(onAuth);
          };
          if(cordova){
            cordovarAuth()
          }
